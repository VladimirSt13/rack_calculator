    ...
    <div class="table-container">
      <table class="table battery-results__table" data-js="battery-resultsTable" data-feature="results">
        ...
        <tbody data-js="battery-resultsBody" data-feature="results">
          ${results
            .map((rack, index) => batteryRackRowTemplate({ rack, index, price }))
            .join('')}
        </tbody>
      </table>
    </div>
    ...
    if (rowspanCell) {
      ...
      for (let j = 1; j < rowspan; j++) {
        if (i + j < rows.length) {
          ...
          // Визначаємо, які комітки відносяться до цього рядка
  ...
  rowspanGroups.forEach((group) => {
    group.forEach((item) => {
      item.row.addEventListener('mouseenter', () => {
        // Підсвічуємо всі комітки в цьому рядку
      ...
      item.row.addEventListener('mouseleave', () => {
        // Знімаємо підсвітку з усіх коміток
        group.forEach((g) => {
