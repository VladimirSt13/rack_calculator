...
import type {
  ...
  SpanComponent,
} from "@/features/rack/types/rack.types";
...
const RackForm: React.FC = () => {
  ...
  const supportsOptions = React.useMemo(
    () =>
      supportsComponents.map((s: SupportComponent) => ({
        value: s.code,
        label: s.name,
      })),
    [supportsComponents],
  ...
  const verticalSupportsOptions = React.useMemo(
    () =>
      verticalSupportsComponents.map((s: VerticalSupportComponent) => ({
        value: s.code,
        label: s.name,
      })),
  ...
  const spanOptions = React.useMemo(
    () =>
      spansComponents.map((s: SpanComponent) => ({
        value: s.code,
        label: s.name || s.code,
      })),
    [spansComponents],
    ...
    <CardContent className="w-full px-0">
      ...
      <FormSectionsGroup>
        ...
        <FormSection title="Геометрія">
          ...
          <FieldRow
            label="Вертикальна опора"
            htmlFor="rack-verticalSupports"
            required={floors > 1}
          >
            <FieldRowSelect
