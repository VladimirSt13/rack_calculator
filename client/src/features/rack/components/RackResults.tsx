...
import {
  useRackResultsStore,
  type RackCalculationResult,
} from "@/features/rack/resultsStore";
...
const RackResults: React.FC<RackResultsProps> = memo(
  ({ isLoading = false }) => {
...
const PreambleCard: React.FC<PreambleCardProps> = memo(({ result }) => {
  ...
  // Кількість балок
  const totalBeams = Array.isArray(beams)
    ? beams.reduce((sum, b) => sum + b.amount, 0)
    : beams.amount || 0;
...
const ComponentsTableCard: React.FC<ComponentsTableCardProps> = memo(
  ({ result, onAddToSet }) => {
    // Збираємо всі компоненти в один масив
    const allComponents = Object.values(result.components).flatMap((items) =>
            ...
            {(!result.prices || result.prices.length === 0) &&
              result.total == null && (
                <div className="flex justify-between items-center py-2">
                  ...
                  <PriceDisplay
                    value={result.totalWithoutIsolators}
                    className="font-medium"
                  />
                </div>
              )}
