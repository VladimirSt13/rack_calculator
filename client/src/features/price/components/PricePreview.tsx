...
type SupportItem =
  | SupportEdgeItem
  | SupportIntermediateItem
  | (PriceItemBase & { edge?: never; intermediate?: never });
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription>
              <ul className="list-disc list-inside space-y-1 mt-2">
                {data.errors
                  .slice(0, 5)
                  .map((error: PriceError, index: number) => (
              ...
              {data.errors.length > 5 && (
                <p className="text-sm mt-2">
                  Ще {data.errors.length - 5} помилок...
                </p>
              )}
            </AlertDescription>
          </Alert>
        ...
        <div className="space-y-6">
          {Object.entries(data)
            ...
            .map(([category, items]) => {
              ...
              const categoryData = Object.entries(items || {}).map(
                ([code, item]) => ({
                  code,
                  ...item,
                }),
              ...
              if (category === "supports") {
                ...
                const supportEntries = Object.entries(items || {}) as Array<
                  [string, SupportItem]
                >;
                              ...
                              rows.push(
                                ...
                                >
                                  ...
                                  <td className="px-4 py-2 text-right">
                                    {typeof item.edge.price === "number"
                                      ? item.edge.price.toFixed(2)
                                      : "—"}
                                  </td>
                                  <td className="px-4 py-2 text-right text-muted-foreground">
                                    {item.edge.weight
                                      ? item.edge.weight.toFixed(2)
                                      : "—"}
                                  </td>
                                ...
                                >
                                  ...
                                  <td className="px-4 py-2 text-right text-muted-foreground">
                                    {item.intermediate.weight
                                      ? item.intermediate.weight.toFixed(2)
                                      : "—"}
                                  </td>
                                  <td className="px-4 py-2 text-muted-foreground text-xs max-w-xs truncate">
