...
const profileSchema = z
  .object({
    currentPassword: z.string().min(6, 'Пароль має бути не менше 6 символів').optional(),
...
export const ProfilePage: React.FC = () => {
  const { user, changePassword, logout, isAdmin } = useAuthStore();
  ...
  const changePasswordMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast.error((error as any).response?.data?.error || 'Помилка зміни пароля');
    },
  });
  ...
  return (
    <div className='container mx-auto py-8 px-4 max-w-2xl'>
          ...
          <div>
            ...
            <p className='text-sm mt-1'>
              <span className='inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800'>
                {isAdmin() ? 'Адміністратор' : user?.roleName?.toUpperCase() === 'MANAGER' ? 'Менеджер' : 'Користувач'}
              </span>
            </p>
          </div>
          <div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className='space-y-4'>
            {isChangingPassword ? (
              <>
                <div className='space-y-2'>
                  ...
                  <Input
                    ...
                    className={errors.currentPassword?.message ? 'border-destructive' : ''}
              ...
              <p className='text-muted-foreground'>Натисніть &quot;Змінити пароль&quot; для зміни пароля</p>
            )}
          </CardContent>
      ...
      <Card className='mt-6 border-destructive'>
        ...
        <CardContent>
          <p className='text-sm text-muted-foreground mb-4'>Після виходу з системи вам потрібно буде знову увійти.</p>
        </CardContent>
        <CardFooter>
