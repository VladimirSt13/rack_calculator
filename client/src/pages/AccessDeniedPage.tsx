...
export const AccessDeniedPage: React.FC = () => {
  const { user, accessToken, isAdmin } = useAuthStore();
  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-gradient-to-br from-destructive/10 to-destructive/5'>
      <div className='w-full max-w-md p-8 space-y-6 bg-background rounded-lg shadow-lg border text-center'>
        ...
        {!accessToken ? (
          <>
            <p className='text-muted-foreground'>Вам необхідно увійти в систему для отримання доступу</p>
        ...
        ) : !isAdmin() && user?.roleName?.toUpperCase() === 'USER' ? (
          <>
            ...
            <div className='space-y-2 text-left text-sm text-muted-foreground'>
              ...
              <ol className='list-decimal list-inside space-y-1 ml-2'>
                ...
                <li>
                  Надайте ваш email: <strong className='text-foreground'>{user?.email}</strong>
                </li>
          ...
          <>
            <p className='text-muted-foreground'>У вас немає прав для перегляду цієї сторінки</p>
            <Link to={PUBLIC_ROUTES.LOGIN}>
              <Button variant='outline' className='w-full'>
                <Home className='w-4 h-4 mr-2' />
                На головну
