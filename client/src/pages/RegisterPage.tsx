...
export const RegisterPage: React.FC = () => {
  ...
  const onSubmit = async (data: RegisterForm) => {
    ...
    try {
      ...
      const errorMessage = errorData?.message || errorData?.error?.message || 'Помилка реєстрації';

      // Спеціальна обробка для існуючого користувача
      if (errorCode === 'USER_EXISTS' || errorCode === 'BAD_REQUEST') {
    ...
    <div className='min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5 py-12 px-4'>
      <div className='w-full max-w-md p-8 space-y-6 bg-background rounded-lg shadow-lg border'>
        <div className='space-y-2 text-center'>
          ...
          <p className='text-muted-foreground'>Створіть акаунт для доступу до системи</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
          <div className='space-y-2'>
            ...
            <p className='text-xs text-muted-foreground'>Використовуйте корпоративну пошту</p>
          </div>
