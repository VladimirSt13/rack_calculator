...
export const MyRackSetsPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      showDeleted ? rackSetsApi.getDeleted() : rackSetsApi.getAll(),
  });
  ...
  const restoreMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast.error(
        (error as any).response?.data?.error || "Помилка відновлення",
      );
    },
  });
  ...
  const filteredSets = mySets?.filter((set) => {
    ...
    return (
      ...
      (set.object_name &&
        set.object_name.toLowerCase().includes(searchLower)) ||
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <RackSetsTable
        ...
        emptyMessage={
          showDeleted
            ? "У вас немає видалених комплектів"
