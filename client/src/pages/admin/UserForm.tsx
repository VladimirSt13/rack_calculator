...
export const UserForm: React.FC<UserFormProps> = ({ user, onClose, onSuccess }) => {
  ...
  const {
    ...
    defaultValues: {
      email: user?.email || '',
      role: (user?.roleName?.toLowerCase() as 'admin' | 'manager' | 'user') || 'user',
      price_types: user?.permissions?.price_types || [],
    },
  });

  const createMutation = useMutation({
    mutationFn: (data: UserForm & { password: string }) => usersApi.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...data }: { id: number } & Partial<UserForm>) => usersApi.update(id, data),
    onSuccess: () => {
            ...
            <label key={type.value} className='flex items-center space-x-2 cursor-pointer'>
              <input
                ...
                onChange={(e) => {
                  ...
                  const updated = e.target.checked ? [...current, type.value] : current.filter((v) => v !== type.value);
