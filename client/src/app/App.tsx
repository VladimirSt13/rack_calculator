...
const Header: React.FC = () => {
  ...
  const { user, logout, accessToken, isAdmin } = useAuthStore();
        ...
        <div className='flex items-center gap-2 sm:gap-4'>
          <nav className='header__nav' aria-label='Головна навігація'>
            <ul className='flex list-none gap-2 sm:gap-4 m-0 p-0'>
              ...
              <li key={PROTECTED_ROUTES.MY_SETS}>
                <Link
                  to={PROTECTED_ROUTES.MY_SETS}
                  className={cn(
                    'px-3 py-2 rounded-md transition-fast font-medium text-sm sm:text-base',
                    location.pathname === PROTECTED_ROUTES.MY_SETS
                      ? 'bg-primary-foreground/10 text-primary-foreground underline'
                      : 'text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/10',
                  )}
                >
              ...
              {(isAdmin() || user?.roleName?.toUpperCase() === 'MANAGER') && (
        ...
        <div className='app min-h-screen flex flex-col bg-background'>
          ...
          <main className='main flex-1'>
            <Routes>
              ...
              {/* Адмін маршрути - потребують дозволів admin */}
              <Route
                path='/admin'
                element={
                  <ProtectedRoute
                    requiredPermissions={['USERS_READ', 'ROLES_READ', 'PRICES_READ', 'AUDIT_READ']}
                    requireActive
                  >
                    <AdminDashboard />
                  </ProtectedRoute>
              ...
              <Route
                path='/admin/users'
                element={
                  <ProtectedRoute requiredPermissions={['USERS_READ', 'USERS_UPDATE']} requireActive>
                    <UserManagement />
                  </ProtectedRoute>
                }
              />
              <Route
                ...
                element={
                  <ProtectedRoute requiredPermissions={['RACK_SETS_READ', 'RACK_SETS_DELETE']} requireActive>
                    <RackSetsList />
                  </ProtectedRoute>
                }
              />
              <Route
                path='/admin/audit'
                element={
                  <ProtectedRoute requiredPermissions={['AUDIT_READ']} requireActive>
                    <AuditLogPage />
                  </ProtectedRoute>
                }
              />
              <Route
                path='/admin/price'
                element={
                  <ProtectedRoute requiredPermissions={['PRICES_READ', 'PRICES_UPDATE']} requireActive>
                    <PriceManagementPage />
                  </ProtectedRoute>
                }
              />
              <Route
                path='/admin/roles'
                element={
                  <ProtectedRoute requiredPermissions={['ROLES_READ', 'ROLES_UPDATE']} requireActive>
                    <RolesManagementPage />
                  </ProtectedRoute>
                }
              />
              ...
              <Route
                path='/my-sets'
                element={
                  <ProtectedRoute requiredPermissions={['RACK_SETS_READ']} requireActive>
                    <MyRackSetsPage />
                  </ProtectedRoute>
                }
              />
              ...
              <Route
                ...
                element={
                  <ProtectedRoute requiredPermissions={['RACK_SETS_CREATE']} requireActive>
                    <RackPage />
                  </ProtectedRoute>
                }
              />
