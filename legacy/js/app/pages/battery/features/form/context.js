 ...
 * @param {Function} onCalculate - callback для розрахунку (приймає formValues)
...
export const createBatteryFormContext = (onCalculate) =>
  createFeatureContext({
    ...
    createActions: (state) => ({
       ...
       * Отримує поточні значення з state та викликає callback
       */
      calculate: () => {
        // Отримуємо поточні значення з state
        const formValues = state.get().form;
