// js/app/pages/battery/core/rackBuilder.js
import { defaultFloorGap, defaultSupportHeight, standardHeights, standardSpans, standardWidths } from './constants.js';
 ...
 * @param {Object} params
 * @param {Object} params.element - { length, width, height, weight }
 * @param {number} params.totalCount
 * @param {number} params.gap
 * @param {number} [params.rows] - обрана кількість рядів (1|2)
 * @param {number} [params.floors] - обрана кількість поверхів (1|2|3)
 * @param {string} [params.supportType] - 'straight'|'step'
 * @param {Object} [params.price] - прайс-лист для розрахунку вартості
...
export const generateRackVariants = ({
  ...
  gap,
  rows: selectedRows = 1,
  floors: selectedFloors = 1,
  supportType = 'straight',
  price = null,
}) => {
  const configs = [];

  // Фільтрація за обраними параметрами — генеруємо тільки обрану конфігурацію
  ...
  const optimizedSpans = optimizeRacks(
    ...
    5, // TOP-5
    price, // прайс для розрахунку вартості
  );

  configs.push({
    ...config,
    supportType,
    topSpans: optimizedSpans, // масив із TOP-5 варіантів
  });
