 ...
 * @param buffer - Buffer Excel файлу
 * @returns Розпарсені дані
 */
export const parsePriceExcel = async (buffer: Buffer): Promise<ParsedPriceData> => {
  ...
  if (!worksheet) {
    result.errors.push({ row: 0, message: 'Worksheet not found' });
    return result;
  }
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    if (currentCategory === 'supports' && (nameMatch || isEdge || isIntermediate)) {
      let codeStr: string;
      let typeShort: 'edge' | 'intermediate';
      ...
      result[currentCategory as keyof Omit<ParsedPriceData, 'errors'>][codeStr] = {
        code: codeStr,
        name: name.toString(),
