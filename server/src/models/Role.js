...
export class Role extends BaseModel {
  ...
  static async findActive() {
    const db = await this.getDb();
    const rows = db.prepare('SELECT * FROM roles WHERE is_active = 1 ORDER BY id').all();
  ...
  static async findDefault() {
    const db = await this.getDb();
    const row = db.prepare('SELECT * FROM roles WHERE is_default = 1 AND is_active = 1').get();
    return row ? new Role(row) : null;
  }
  ...
  async setPriceTypes(priceTypes) {
    ...
    if (priceTypes.length > 0) {
      const insert = db.prepare('INSERT INTO role_price_types (role_id, price_type) VALUES (?, ?)');
