    ...
    if (priceData) {
      ...
      const rackSetsWithPrices = await Promise.all(
        rackSets.map(async (rackSet) => {
          const racksWithPrices = await rackSet.getRacksWithPrices(priceData, req.user);
    ...
    if (priceData && rack_items.length > 0) {
      const racksWithPrices = await Promise.all(
        rack_items.map(async (item) => {
          const rackData = await getRackDataFromConfig(item.rackConfigId, priceData, req.user);
          return rackData ? { ...rackData, quantity: item.quantity } : null;
        })
      );
      totalCostSnapshot = calculateRackSetTotal(racksWithPrices.filter(Boolean));
    }
  ...
  try {
    ...
    if (rack_items !== undefined) {
      ...
      if (priceData && rack_items.length > 0) {
        ...
        updateData.totalCostSnapshot = calculateRackSetTotal(racksWithPrices.filter(Boolean));
      }
    }
...
export const restoreRackSet = async (req, res, next) => {
  try {
    ...
    const rackSet = await RackSet.findById(id, { userId, isAdmin, includeDeleted: true });
...
export const hardDeleteRackSet = async (req, res, next) => {
  try {
    ...
    const rackSet = await RackSet.findById(id, { isAdmin: true, includeDeleted: true });
