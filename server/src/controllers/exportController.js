...
const createRackSetWorksheet = (
  ...
  if (additionalInfo.date) {
    ...
    worksheet.getCell("B2").value =
      typeof additionalInfo.date === "string"
        ? additionalInfo.date
  ...
  racks.forEach((rack, index) => {
    ...
    const noIsolatorsPrice =
      rack.prices?.find(
        (p) => p.type === "без_ізоляторів" || p.type === "no_isolators",
      )?.value || 0;
    const zeroPrice =
      ...
      Object.entries(rack.components).forEach(([category, items]) => {
        ...
        itemsArray.forEach((item) => {
          ...
          if (showPrices) {
            ...
            compRow.values = [compIndex++, item.name, item.amount, totalAmount];
          }
...
export const exportRackSet = async (req, res, next) => {
  try {
    ...
    // Отримати комплект з повними даними
    const rackSetWithDetails = await exportService.getRackSetWithDetails(
      id,
      { userId, isAdmin },
      req.user,
    );

    if (!rackSetWithDetails) {
      return res.status(404).json({ error: "Rack set not found" });
    }

    const { racks, created_at, name } = rackSetWithDetails;
    ...
    createRackSetWorksheet(workbook, name, racks, showPrices, {
      date: created_at,
    ...
    const filename = showPrices ? `${name}_з_цінами.xlsx` : `${name}.xlsx`;
    res.setHeader(
      "Content-Disposition",
...
export const exportNewRackSet = async (req, res, next) => {
  try {
    ...
    const racks = await exportService.getNewRackSetData(rack_items, req.user);
    ...
    // Створити worksheet з даними
    createRackSetWorksheet(
      workbook,
      "Новий комплект стелажів",
      racks,
      showPrices,
      {
        date: new Date(),
    ...
    if (error.message === "Rack items array is required and cannot be empty") {
