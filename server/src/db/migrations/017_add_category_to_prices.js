    ...
    try {
      ...
      for (const record of allPrices) {
        try {
          ...
          if (
            data.vertical_supports &&
            Object.keys(data.vertical_supports).length > 0
          ) {
            categories.add("vertical_supports");
          }
          if (
            data.diagonal_brace &&
            Object.keys(data.diagonal_brace).length > 0
          ) {
          ...
          console.log(
            `[Migration 017] Error processing record ${record.id}:`,
            e.message,
          );
        }
      }
