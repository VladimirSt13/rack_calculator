...
export const runMigrations = async (db) => {
  ...
  if (migrated === 0) {
    ...
    console.log(`[Migrations] ✓ ${migrated} migration(s) completed successfully`);
  }
};
...
export const rollbackMigration = async () => {
  ...
  try {
    ...
    if (typeof migration.down !== 'function') {
      console.warn(`[Migrations] Warning: ${lastMigration} has no down() function`);
      return;
    }
    ...
    console.error(`[Migrations] ✗ ${lastMigration} rollback failed:`, error.message);
    throw error;
  }
};
...
export const checkMigrationStatus = async () => {
  ...
  for (const file of files) {
    ...
    const date = executed.includes(file)
      ? db.prepare('SELECT executed_at FROM _migrations WHERE name = ?').get(file).executed_at
      : '-';
