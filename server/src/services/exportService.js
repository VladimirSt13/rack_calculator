...
export const getRackDataFromConfig = async (rackConfigId, priceData, user) => {
  ...
  if (Array.isArray(rackConfig.spans) && typeof rackConfig.spans[0] === 'number') {
    ...
    rackConfig.spans = Array.from(spansMap.entries()).map(([item, quantity]) => ({ item, quantity }));
  }
...
export const getRackSetWithDetails = async (id, { userId, isAdmin }, user) => {
  ...
  let rackSet;
  if (isAdmin) {
    rackSet = db.prepare('SELECT id, name, object_name, description, rack_items, total_cost_snapshot, created_at FROM rack_sets WHERE id = ?').get(id);
  } else {
    rackSet = db.prepare('SELECT id, name, object_name, description, rack_items, total_cost_snapshot, created_at FROM rack_sets WHERE id = ? AND user_id = ?').get(id, userId);
  }
  ...
  const racks = (await Promise.all(rackItems.map(async (item) => {
    const rackData = await getRackDataFromConfig(item.rackConfigId, priceData, user);
